...
import (
	...
	"github.com/lucs-t/tshell/server/utils"
)
...
func WriteSSHinfoFiles(info models.SSHInfo) error {
	...
	if err != nil {
		return fmt.Errorf("failed to check SSH info file: %w", err)
	}

	// 读取现有的 SSH 信息
	infos, err := ReadSSHinfoFiles()
	if err != nil {
		return fmt.Errorf("failed to read SSH info files: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("failed to open SSH info file: %w", err)
	}
	defer file.Close()
	...
	if err != nil {
		return fmt.Errorf("failed to encode SSH info: %w", err)
	}
...
func ReadSSHinfoFiles() ([]models.SSHInfo, error) {
	...
	err := CheckFile(sshInfoFile)
	if err != nil {
		return nil, fmt.Errorf("failed to check SSH info file: %w", err)
	}
	...
	if err != nil {
		...
		return nil, fmt.Errorf("failed to open SSH info file: %w", err)
	}
	defer file.Close()
	...
	if err != nil {
		return nil, fmt.Errorf("failed to decode SSH info: %w", err)
	}
...
func WriteSSHconfigFiles(config models.SSHConfig) error {
	...
	if err != nil {
		return fmt.Errorf("failed to encode SSH config: %w", err)
	}
...
func ReadSSHconfigFiles() (models.SSHConfig, error) {
	...
	err := CheckFile(sshConfigFile)
	if err != nil {
		return models.SSHConfig{}, fmt.Errorf("failed to check SSH config file: %w", err)
	}
	...
	if err != nil {
		...
		return models.SSHConfig{}, fmt.Errorf("failed to open SSH config file: %w", err)
	}
	defer file.Close()
	...
	if err != nil {
		return models.SSHConfig{}, fmt.Errorf("failed to decode SSH config: %w", err)
	}
